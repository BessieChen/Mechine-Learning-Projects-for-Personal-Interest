...
def predict_price_Linear(dates, X, Y, x=3):
    ...
    plt.plot(dates, linear_mod.predict(X), color='red',
...
if __name__ == "__main__":
    ...
    for ticker in ticker_arr[0]:
        ...
        forest = ExtraTreesClassifier(n_estimators=250,
                                      random_state=0)
        print stock_feature_df.values[0]
        print stock_returns.values[0]
        print stock_feature_df.shape
        print stock_returns.shape
        print stock_feature_df.axes
        print stock_returns.axes
        print stock_movement.shape
        print stock_movement.axes

        aa = pd.DataFrame()
        aa["Lag0"] = stock_returns["Lag0"]
        aa = np.ravel(aa)

        predicted_price = predict_price_Linear(stock_feature_df.index, stock_feature_df.values, aa)
